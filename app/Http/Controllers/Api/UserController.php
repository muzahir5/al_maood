...
class UserController extends Controller
{
    ...
    {        
        ...
        if($audio){
            ...
            $status = 'success';
            $relavent_audio = Audio::where(['category'=> $audio->category, 'show_to'=>$audio->show_to])->where('show_to','!=',$audio->show_to)->get();
        }
    ...
    public function userEarning(Request $request)
    {
        ...
        if($userEarnig)
        {
            ...
            $status = 'success'; $message = "User Transaction don $request->transaction ";
        }
        return response()->json([ 'status' => "success", 'data' => $message ]);
